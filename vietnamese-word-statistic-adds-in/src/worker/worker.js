...
function analyze(paragraphs) {
  ...
  let sino_count = countSinoWords(words);


  return {letter_count, character_count, syllable_count, word_count, sentence_count, sino_count, words};
}
...
self.addEventListener('message', e => {
  ...
  for (let i = 0; i < total_paragraph_count; i += 10) {
    const {letter_count, character_count, syllable_count, word_count, sentence_count, sino_count, words} = analyze(paragraphs.slice(i, i + 10));
    ...
    total_sino_count += sino_count;
    
    let readabilityLevel = total_sino_count;
    // evaluateReadability(total_sino_count, total_word_count, total_sentence_count);
